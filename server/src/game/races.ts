...
export const RACES: Record<RaceKey, RaceDef> = {
  humanos: {
    ...
    lore: "Engenharia robusta e blindagem pesada. As naves humanas aguentam muito mais castigo antes de cair — confiaveis na linha de frente.",
...
export function publicRaces() {
  return RACE_KEYS.map((k) => {
    const r = RACES[k];
    return { key: r.key, name: r.name, tagline: r.tagline, lore: r.lore };
  });
}
